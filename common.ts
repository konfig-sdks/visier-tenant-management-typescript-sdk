...
import { readableStreamToString, VisierTenantManagementError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new VisierTenantManagementError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new VisierTenantManagementError(e, e.response?.data)
                }
                if (innerError instanceof VisierTenantManagementError) {
                    // Got "VisierTenantManagementError" from the above try block
                    throw innerError;
                }
